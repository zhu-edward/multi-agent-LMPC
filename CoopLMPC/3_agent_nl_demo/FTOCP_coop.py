	...
	def solve(self, x0, abs_t, xf=None, expl_con=None, SS=None, Qfun=None, CVX=False, verbose=False):
		...
		if problem.status != cp.OPTIMAL:
			if problem.status == cp.INFEASIBLE:
				print('Optimization was infeasible for time %i' % abs_t)
			elif problem.status == cp.UNBOUNDED:
				print('Optimization was unbounded for time %i' % abs_t)
			elif problem.status == cp.INFEASIBLE_INACCURATE:
				print('Optimization was infeasible inaccurate for time %i' % abs_t)
			elif problem.status == cp.UNBOUNDED_INACCURATE:
				print('Optimization was unbounded inaccurate for time %i' % abs_t)
			elif problem.status == cp.OPTIMAL_INACCURATE:
				print('Optimization was optimal inaccurate for time %i' % abs_t)
...
class agent_FHOCP(object):

	def __init__(self, Q, P, R, Rd, N, agent, n_model_per_control=1, obstacles=[], x_ref=None):
		self.Q = Q
		self.P = P
		self.R = R
		self.N = N
		...
		self.n_model_per_control = n_model_per_control
	...
	def solve(self, x_0, abs_t, verbose=False):
		...
		for i in range(self.N):
			...
			# if self.n_model_per_control > 1:
			# 	A_pows = [la.matrix_power(A, i) for i in range(self.n_model_per_control+1)]
			# 	A_pows = np.concatenate(A_pows, axis=1)
			# 	B_mat = np.tile(B, (self.n_model_per_control,1))
			# 	c_vec = np.tile(c, self.n_model_per_control)
			# 	A = A_pows[:,-self.n_x:]
			# 	B = A_pows[:,:-self.n_x].dot(B_mat)
			# 	c = A_pows[:,:-self.n_x].dot(c_vec)

			constr += [x[:,i+1] == A*x[:,i] + B*u[:,i] + c]
			...
			cost += cp.quad_form(x[:,i]-self.x_ref, self.Q) + cp.quad_form(u[:,i], self.R)
			constr += [cp.abs(u[0,i]-self.u_preds[0,0,-1]) <= 0.3*self.agent.dt] # Steering rate
			constr += [cp.abs(u[1,i]-self.u_preds[1,0,-1]) <= 3.0*self.agent.dt] # Throttle rate
			if i < self.N-1:
				constr += [cp.abs(u[0,i+1]-u[0,i]) <= 0.3*self.agent.dt] # Steering rate
				constr += [cp.abs(u[1,i+1]-u[1,i]) <= 3.0*self.agent.dt] # Throttle rate
				cost += cp.quad_form(u[:,i+1]-u[:,i], self.Rd)

		# Terminal state constraints
		if self.F is not None:
			constr += [self.F*x[:,self.N] <= self.b]

		cost += cp.quad_form(x[:,self.N]-self.x_ref, self.P)
